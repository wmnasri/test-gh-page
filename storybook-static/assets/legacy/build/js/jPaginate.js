    ...
    $.fn.jPaginate = function(options) {
        ...
        return this.each(function() {
            ...
            function createPagination(curr) {
                ...
                var page_position = "<li id='PaginatePosition'>Page "+curr+" sur "+number_of_pages+"</li>";
                end = page_position+"</ul>";
                var after = number_of_pages - options.after + 1;
                var pagi_range = paginationCalculator(curr);
		
                //console.log("pagi_start " + pagi_range.start);
                //console.log("pagi_end   " + pagi_range.end);
				...
				if (i == curr) { 
                                    if(i >= 10)
                                        items += '<li><a class="'+options.active+" "+options.activeDouble+'" title="'+i+'">'+i+'</a></li>';
                                    else 
                                        items += '<li><a class="'+options.active+'" title="'+i+'">'+i+'</a></li>';
                                } 
                        	else {
                                    if(i >= 10)
                                        items += '<li><a href="#" class="goto gotoDouble" title="'+i+'">'+i+'</a></li>';                                        
                                    else 
                                        items += '<li><a href="#" class="goto" title="'+i+'">'+i+'</a></li>';
                                }
                        }
                        else if (curr == pagi_range.end){
                                items += '<li><a class="'+options.active+'" title="'+i+'">'+i+'</a></li>';
...
((expiredays==null)?";Secure":";Secure;expires="+exdate.toUTCString());
			}
			...
			function paginationCalculator(curr)  {
				...
				var upper_limit = number_of_pages - options.nav_items;
                                ...
                                if( curr >= 1 && curr < 10 ){
                                    var start = 1;
                                    var end = Math.min(options.nav_items, number_of_pages);
                                }
                                else {
                                    var start = Math.floor(curr / options.nav_items) * options.nav_items;
                                ...
                                // Start 
                                if (curr > half){ 
                                    var start = Math.max( Math.min(curr - half, upper_limit), 0 );
                                } else {
                                    var start = 0;
                                }
                                ...
                                // End
                                if (curr > half){
                                    var end = Math.min(curr + half + (options.nav_items % 2), number_of_pages);
                                } else {
                                    var end = Math.min(curr + half + (options.nav_items % 2), number_of_pages):Math.min(options.nav_items, number_of_pages);
                                }*/
            ...
            $(".goto_next").live("click", function(e) {
                ...
                var newcurr = parseInt($(".jPaginate").find(".active").attr("title")) + 1;
